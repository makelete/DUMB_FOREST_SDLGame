...
int GameOver::loadGameOver(SDL_Renderer* renderer, TTF_Font* font) {
	...
	BuildItemGO(0, "Play Again", 650, 600, Text::GREEN, renderer, font);
    ...
    while(true){
		...
		SDL_Rect renderQuad = { xp,yp,1600,800};
		SDL_RenderCopy(renderer, texture, NULL, &renderQuad);
		text_go[0].RenderText(renderer, 650,600);
			...
			case SDL_MOUSEMOTION:
			{
				...
				for (int i = 0;i < 1;++i) {
					if (CheckFocusWithRectGO(xG, yG, text_go[i].GetRect())) {
						...
						{
							selectedGO[i] = false;
							ChangeColorGO(i, Text::GREEN, renderer, font);
						}
					}
				}
			}
