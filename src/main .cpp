...
bool init() {
	...
	gwindow = SDL_CreateWindow("DUMB_FOREST", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1600, 800, SDL_WINDOW_SHOWN);
	if (gwindow == NULL) {
		success = false;
	}
	else {
		...
		mainfont = TTF_OpenFont("fonts/HomemadeApple.ttf", 50);
		if (mainfont == NULL) 
		{
			success = false;
		}
...
int main(int arcs, char* argv[]) {
	...
	Player player1;
	player1.LoadImg("assets/player1_stay_right.png", gscreen);
	player1.setclip();

	Player player2;
	player2.LoadImg("assets/player2_stay_right.png", gscreen);
	player2.setclip();

	player1.setPos(230, 620);
	player2.setPos(180, 620);

	OtherObj cutemus;
	cutemus.setPos(150,412);
    cutemus.LoadImg("assets/mushroom.png", gscreen, 48);
	cutemus.getNum(4);
	cutemus.setclip();

	OtherObj gate;
	gate.setPos(1350, 160);
	gate.LoadImg("assets/gate.png",gscreen, 32);
	gate.getNum(14);
	gate.setclip();

	Text score_text_1;
	score_text_1.SetColor(Text::WHITE);
	int score_value_1 = 0;
    ...
    {	
		...
		if(level == 1)
		{
			...
			player1.setPos(280, 580);
			player2.setPos(300, 580);
			cutemus.setPos(150, 412);
			gate.setPos(1350, 160);
	...
	while (!quit) {
		if (state==isGameover)
		{
			...
			if (_gameover.loadGameOver(gscreen, mainfont)==0) {
                ...
                {
                	background1.Render(gscreen, NULL);
					player1.setPos(200, 580);
					player2.setPos(260, 580);
					cutemus.setPos(150, 412);
					gate.setPos(1350, 160);
				}
				if (level == 2)
		...
		else if (state == isPlaying)
		{
    		...
    		score_text_2.LoadFont(mainfont, gscreen);
			score_text_1.RenderText(gscreen, 200, 2);
			score_text_2.RenderText(gscreen, 1050, 2);
